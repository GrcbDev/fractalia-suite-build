...
import { AttacksimulatorService } from 'src/app/modules/home/attack-simulator-service/shared/attacksimulator.service';
...
import { LoginToAcronisURL } from 'src/app/modules/home/user-security/shared/cac-consts';
...
export class AutomaticActivationComponent implements OnInit {
  ...
  public showTrendMicro: boolean = false;
  public showTrendMicroEnd: boolean = false;
  public showNoTrendMicro: boolean = false;
  public trendMicroNoLicense: boolean = false;
  public trendMicroDevices: boolean = false;
  public onlyTrendMicroService: boolean = false;
  ...
  closeResumed() {
    ...
    if (this.stepsData[this.currentIndex].totalSteps > 1 && this.stepsData[this.currentIndex].serviceCode == this.currentService) {
      this.advancing = true;
      this.resumed = false;
      if (this.stepsData[this.currentIndex].serviceCode === 'TRE') {
        if (this.stepsData[this.currentIndex].finalized) {
          this.renewStepsData();
        } else {
          this.validateTrendmicro();
        }
  ...
  installableService(service: string) {
    switch (service) {
      ...
      case 'TRE': {
        return (this.trendMicroDevices || this.trendMicroNoLicense && !this.trmGreen) ? true : false
      }
    ...
    await this.userService.getActivationServices().then(
      result => {
        ...
        for (let i = 0; i < tempData.length; i++) {
          if (this.stepsData[i].serviceCode == 'TRE' && this.stepsData[i].currentStep == 0) {
            this.stepsData[i].totalSteps = 1;
          }
          ...
          if (this.stepsData[i].finalized == true) {
            ...
            if (this.stepsData[i].serviceCode == 'TRE') {
              this.trmGreen = true;
            }
          }
          ...
          if (tempData.length == 1 && this.stepsData[i].serviceCode == 'TRE') {
            this.currentService = 'TRE';
            this.onlyTrendMicroService = true;
          }
        ...
        result => {
          ...
          for (let i = 0; i < tempData.length; i++) {
            if (this.currentService == tempData[i].serviceCode) {
              ...
              if (this.currentService == 'TRE') {
                if (tempData[i].currentStep != 0) {
                  if (tempData[i].currentStep >= 1) {
                    this.currentIndex = i
                  }

                  if (tempData[i].finalized == true) {
                    this.showTrendMicroEnd = true;
                    this.trmGreen = true;
                  } else {
                    this.showTrendMicroEnd = true;
                    this.trendMicroDevices = (this.trendmicroservice.trendmicro.devices.length == 0);
                    tempData[i].currentStep = tempData[i].totalSteps
                    tempData[i].finalized = true
                  }
                } else {
                  this.showNoTrendMicro = false;
                  this.trendMicroNoLicense = true;
                  tempData[i].currentStep = 2;
                  tempData[i].finalized = true;
                }
              }
            ...
            if (tempData[i].finalized == false && this.currentService === '') {
              ...
              //console.log('this.currrentservice', this.currentService)
          ...
          // calculate progress before add static steps
          this._totalSteps = this.currentStep;
          //Start code for specific logic for TrendMicro by step 0
          if (this.trendMicroNoLicense) {
            this._totalSteps -= 1;
          }
          //End code for specific logic for TrendMicro by step 0
          this.progressCalc(this._totalSteps, this.totalSteps - 2)
  ...
  async validateTrendmicro() {
    if (!this.onlyTrendMicroService) {
      if (this.stepsData[this.currentIndex + 1].currentStep == 1 ||
        this.stepsData[this.currentIndex + 1].currentStep == 0) {
        this.loading = true;
        this.trendmicroservice.getTrendmicroLicense().pipe(
          tap(res => {
            if (res.created) {
              this.trendmicroservice.trendmicro = res;
              this.advancing = true;
              this.currentIndex += 1;
              this.showTrendMicro = true;
              setTimeout(() => {
                this.advancing = false;
              }, 100);
            } else {
      ...
      if (this.stepsData[this.currentIndex].currentStep == 1 ||
        this.stepsData[this.currentIndex].currentStep == 0) {
        this.loading = true;
        this.trendmicroservice.getTrendmicroLicense().pipe(
          tap(res => {
            if (res.created) {
  ...
  checkLastStep() {
    if (this.currentStep == (this.totalSteps - 1)) {
      ...
      this.currentIndex = 0;
      this.currentStep += 1
      if (this.showTrendMicroEnd) {
        this.showTrendMicroEnd = false;
      }
      setTimeout(() => {
        this.advancing = false;
      }, 100);
