...
export class SignupComponent implements OnInit {
  ...
  private recaptchaV3ServiceSubscription: Subscription;
  ...
  registerCode() {
    this.recaptchaV3ServiceSubscription = this.recaptchaV3Service
      .execute('register')
      .subscribe(
        (token) => {
          this.loading = true;
          ...
          this.userService.registerCode(request).subscribe(
            (result:any) => {
              this.loading = false;
              switch (result) {
                case -2:
