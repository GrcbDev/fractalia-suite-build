...
import { BitDefenderMspResponse } from 'src/app/models/bitDefenderMspResponse';
import { UserService } from 'src/app/services/user.service';

@Injectable({
  providedIn: 'root'
})
export class BitdefendermsmService {
  ...
  msmServicechange$: BehaviorSubject<number> = new BehaviorSubject(null);
  msmServicereload$: BehaviorSubject<number> = new BehaviorSubject(null);
  ...
  emitMSMServiceChange(change: any){
    this.msmServicechange$.next(change);
  }

  emitMSMServiceReload(change: any){
    this.msmServicereload$.next(change);
  }

  public getUserLicenceMSM(partnerSuiteId: string) {
    let params = new HttpParams();
    params = params.set('partnerSuiteId', partnerSuiteId);
    return this.http.get<BitDefenderMspResponse>(environment.apiEndpoint + "api/licence/msm/subscriberid", { params });
  }

  formatBitDefenderMSMData(response: any) {
    this.Acces_central();
  ...
  public setUserLicenceMSM(partnerSuiteId: string, lang: string, partnercode: string) {
    let params = new HttpParams();
    params = params.set('partnerSuiteId', partnerSuiteId);
    params = params.set('lang', lang);
    params = params.set('partnercode', partnercode);
    return this.http.get<BitDefenderMspResponse>(environment.apiEndpoint + "api/licence/msm/subscriber", { params });
  }
  ...
  asignProfileUserToLicence(deviceId: number, userProfileId: number) {
    return this.http.post(environment.apiEndpoint + "api/Licence/msm/asignProfileUserToLicence?deviceId=" + deviceId + "&userProfileId=" + userProfileId, {})
  }
