...
@Component({
  selector: 'app-rgpd-candidates',
  templateUrl: './rgpd-candidates.component.html',
  styleUrls: ['./rgpd-candidates.component.scss']
})
export class RgpdCandidatesComponent implements OnInit {
  @Input() data: any;
  ...
  saveFormData(isEditMode?: boolean) {
    ...
    if(this.questionsForm.valid && this.stepForm.valid) {
      ...
      this.rgpdService.proccessDataToUpdate(RGPDSteps.Candidates, { stepForm : this.stepForm, formValues: formValues, agentList: this.agentList });
  ...
  previusStep() {
    this.rgpdService.emitGoToStep(RGPDSteps.Employees);
  }
