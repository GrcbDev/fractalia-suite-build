...
@Component({
  selector: "app-rgpd-clients",
  templateUrl: "./rgpd-clients.component.html",
  styleUrls: ["./rgpd-clients.component.scss"],
})
export class RgpdClientsComponent implements OnInit {
  @Input() data: any;
  ...
  saveFormData(isEditMode?: boolean) {
    ...
    if(this.questionsForm.valid && this.stepForm.valid) {
      ...
      this.rgpdService.proccessDataToUpdate(RGPDSteps.Clients, { stepForm : this.stepForm, formValues: formValues, agentList: this.agentList });
  ...
  previusStep() {
    this.rgpdService.emitGoToStep(RGPDSteps.EnterpriseData);
  }
