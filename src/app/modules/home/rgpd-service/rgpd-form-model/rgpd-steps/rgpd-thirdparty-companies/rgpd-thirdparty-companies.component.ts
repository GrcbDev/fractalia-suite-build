...
@Component({
  selector: 'app-rgpd-thirdparty-companies',
  templateUrl: './rgpd-thirdparty-companies.component.html',
  styleUrls: ['./rgpd-thirdparty-companies.component.scss']
})
export class RgpdThirdpartyCompaniesComponent implements OnInit {
  @Input() data: any;
  ...
  nextStep() {
    if(this.editMode) {
      this.rgpdService.emitGoToStep(RGPDSteps.VideoSurveillance);
    } else {
      this.saveFormData();
    }
  }
  ...
  saveFormData(isEditMode?: boolean) {
    ...
    if(this.questionsForm.valid && this.stepForm.valid) {
      ...
      this.rgpdService.proccessDataToUpdate(RGPDSteps.ThirdPartyCompanies, { stepForm : this.stepForm, formValues: formValues, agentList: this.agentList });
  ...
  previusStep() {
    this.rgpdService.emitGoToStep(RGPDSteps.Providers);
  }
