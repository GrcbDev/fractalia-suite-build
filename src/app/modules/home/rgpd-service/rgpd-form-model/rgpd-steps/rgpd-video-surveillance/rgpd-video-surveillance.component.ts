...
@Component({
  selector: 'app-rgpd-video-surveillance',
  templateUrl: './rgpd-video-surveillance.component.html',
  styleUrls: ['./rgpd-video-surveillance.component.scss']
})
export class RgpdVideoSurveillanceComponent implements OnInit {
  @Input() data: any;
  ...
  nextStep() {
    if(this.editMode) {
      this.rgpdService.emitGoToStep(RGPDSteps.Success);
    } else {
      this.saveFormData();
    }
  }
  ...
  saveFormData(isEditMode?: boolean) {
    ...
    if(this.questionsForm.valid && this.stepForm.valid) {
      ...
      this.rgpdService.proccessDataToUpdate(RGPDSteps.VideoSurveillance, { stepForm : this.stepForm, formValues: formValues, agentList: this.agentList });
  ...
  previusStep() {
    this.rgpdService.emitGoToStep(RGPDSteps.ThirdPartyCompanies);
  }
