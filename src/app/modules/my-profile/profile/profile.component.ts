...
export class ProfileComponent implements OnInit {
  ...
  public skipWizard: string = '';
  public showRequiredTerms: boolean = false;
  public showRequiredPass: boolean = false;
  ...
  ngOnInit() {
    ...
    this.userService.getUserNotificationConfig(this.user.id).then((result) => {
  ...
  onSubmit() {
    ...
    this.submitted = true;
    if (this.user.name != null) this.user.name = this.user.name.trim();
    if (this.user.surname != null) this.user.surname = this.user.surname.trim();
    if (this.user.email != null) this.user.email = this.user.email.trim();
    if (this.user.eid != null) this.user.eid = this.user.eid.trim();
    if (this.user.phone != null) this.user.phone = this.user.phone.trim();
    ...
    if (this.loginType == 2 || this.loginType == 5) {
      if (!this.isValidString(this.user.eid)) {
        this.message = this.translate
          ...
          .replace(
            "{{identifier}}",
            this.partnerService.partner.uniqueIdentifier
          );
        this.showMessage = true;
        this.loading = false;
        return;
    ...
    var languageUser = this.partnerService.partner.languages.find(
      (l) => l.code == this.user.languageCode
    );
    if (languageUser) this.user.languageId = languageUser.id;
    ...
    this.userService.updateUser(this.user).subscribe(
      () => {
        ...
        if (this.userService.companyUser)
          userUpdated = this.userService.companyUser.userList.find(
            (u) => u.userId == this.userService.currentUserValue.id
          );
        if (userUpdated) {
        ...
        if (
          (this.partnerService.partner.loginType === 2 && this.skipWizard == 'false') ||
        ...
        if (this.user.languageCode != localStorage.getItem("langUser")) {
          localStorage.setItem("langUser", this.user.languageCode);
          this.languageService.setLanguage(this.user.languageCode, true);
        }
        this.showRequiredTerms = false;

        this.emitService.rechargeSidebar.emit(true);
      },
      (error) => {
  ...
  onUpdateProfile() {
    ...
    if (this.partnerService.partner.hasFullProfile) {
      var userFullProfile = {
        ...
        languageId: this.partnerService.partner.languages.find(
          (l) => l.code == this.user.languageCode
        ).id,
      };
      ...
      this.userService
        ...
        .then((result: Result) => {
          ...
          this.userService.currentUserValue.profileImage =
          ...
          this.userService.currentUserValue.languageCode =
            this.user.languageCode;
          this.userService.currentUserValue.passwordChanged =
            this.user.passwordChanged;
  ...
  viewUpdateProfile(result: Result) {
    switch (result.status) {
      case 200:
        ...
        if(
          this.partnerService.partner.loginType === 2 && this.skipWizard == 'true'
        ) 
          this.router.navigate(["/auto-activation"]);
  ...
  copyToClipboard(event: any) {
    ...
    const copyTarget = document.getElementById(
      "passwordExample"
    ) as HTMLInputElement;
    copyTarget.select();
  ...
  private loadElearningNotification() {
    this.userService
      ...
      .pipe(
        tap((data: UserPost) =>
          this.userService.setElearningNotification(data)
        ),
        tap(
          (elearning: UserPost) =>
            (this.elearningNotification = elearning.state == 1 ? true : false)
        ),
        catchError(async () => {})
      )
      .subscribe();
