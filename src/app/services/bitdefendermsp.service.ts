...
export class BitdefendermspService {
  ...
  bmsServicechange$: BehaviorSubject<number> = new BehaviorSubject(null);
  bmsServicereload$: BehaviorSubject<number> = new BehaviorSubject(null);
  ...
  emitBmsServiceReload(change: any){
    this.bmsServicereload$.next(change);
  }

  public getUserLicenceMspObservable(partnerSuiteId: string) {
    let params = new HttpParams();
    params = params.set('partnerSuiteId', partnerSuiteId);
    return this.http.get<BitDefenderMspResponse>(environment.apiEndpoint + "api/licence/msp/subscriberid", { params });
  }
  ...
  formatBitDefenderMSPData(response: any) {
    this.Acces_central();
    this.bitdefenderResponse = response;
    (this.bitdefenderResponse.devices != null && this.bitdefenderResponse.devices.length > 0)? this.hasBitDefenderMspDevices = true : this.hasBitDefenderMspDevices = false;
    if (this.chartData.length > 0)
  ...
  public setUserLicenceMSPObservable(partnerSuiteId: string, lang: string, partnercode: string) {
    let params = new HttpParams();
    params = params.set('partnerSuiteId', partnerSuiteId);
    params = params.set('lang', lang);
    params = params.set('partnercode', partnercode);
    return this.http.get<BitDefenderMspResponse>(environment.apiEndpoint + "api/licence/msp/subscriber", { params });
  }
  ...
  asignProfileUserToLicence(deviceId: number, userProfileId: number) {
    return this.http.post(environment.apiEndpoint + "api/Licence/msp/asignProfileUserToLicence?deviceId=" + deviceId + "&userProfileId=" + userProfileId, {})
  }
}
